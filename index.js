...
async function starts() {
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Tchau @${num.split('@')[0]} tchau otaru`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				wait: '鈱� Editando, aguade... 鈱�',
				success: '鉁旓笍 Deu certo, ufa kk 鉁旓笍',
				error: {
					stick: '鈿狅笍 Falha, ocorreu um erro ao converter a imagem em figurinha 鈿狅笍',
					Iv: '鉂� Link tidak valid 鉂�'
				},
				only: {
					group: '鉂� Este comando s贸 pode ser usado em grupos! 鉂�',
					ownerG: '鈿狅笍 Este comando s贸 pode ser usado pelo dono do bot! 馃槀',
					ownerB: '鉂� Este comando s贸 pode ser usado pelo propriet谩rio do bot! 鉂�',
					admin: '鈿狅笍 Este comando s贸 pode ser usado pelo o adm! 馃槤',
					Badmin: '鉂� Este comando s贸 pode ser usado quando eu virar adm porra! 鉂�'
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["15139826955@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'sticker':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`鉂� Falha ao converter ${tipe} para figurinha`)
							})
							.on('end', function () {
								console.log('Finish')
								client.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
					...
					/*} else if ((isMedia || isQuotedImage) && colors.includes(args[0])) {
						...
						reply(`Envie fotos com legendas ${prefix}sticker ou tags de imagem que j谩 foram enviadas`)
					}
					break
				case 'gtts':
					if (args.length < 1) return client.sendMessage(from, 'Qual o c贸digo de idioma, cara?', text, {quoted: mek})
					const gtts = require('./lib/gtts')(args[0])
					if (args.length < 2) return client.sendMessage(from, 'Cad锚 o texto garai?', text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
					? reply('A maior parte do texto 茅 merda tio')
				...
				case 'porno':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'Adm proibiu porno no gp馃檮'})
					break
				case 'dono':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/d1M6lOz.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* AlviinPS\n*WPP:* wa.me/+557184274015\n*INSTA:* @alviinps \n\n\n*Digite *.bot* para ver comandos basicos para criar um bot'})
					break
				case 'belle2':
					memein = await kagApi.memeindo()
				...
				case 'loli1':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'hmm safado, ent茫o quer ver loli?'})
					break
				case 'hentai':
					memein = await kagApi.memeindo()
					buffer = await getBuffer(`https://i.imgur.com/8U9GwX4.jpg`)
					client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ih ala, o cara bate pra 2d 馃槀'})
					break
				case 'bomdia':
					memein = await kagApi.memeindo()
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '锔�*canal do zeus:*\n\n https://bit.ly/3iIlHmc'})
					break
				case 'mia1':
					memein = await kagApi.memeindo()
				...
				case 'mia2':
					...
					client.sendMessage(from, buffer, video, {quoted: mek, caption: 'use o .sticker para ver o gif da mia锔�'})
					break
				case 'memeindo':
					memein = await kagApi.memeindo()
				...
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'heher boy馃檳'})
					})
					break
				case 'nsfwloli':
				...
				case 'yt2mp3':
					...
					break
				case 'ytsearch':
					if (args.length < 1) return reply('O que voc锚 est谩 procurando? pau?')
                ...
                case 'marcar2':
                 ...
                 case 'marcar3':
					...
					break
				case 'limpar':
					if (!isOwner) return reply('Comando so funfa, se voc锚 for o dono do bot 馃槨')
					...
					break
				case 'ts':
					if (!isOwner) return reply('Quem 茅 voc锚 lek?')
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ ISSO E UMA TRANSMISS脙O ]\n\n${body.slice(4)}`})
						}
						reply('Transmiss茫o enviada com sucesso')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ ISSO E UMA TRANSMISS脙O ]\n\n${body.slice(4)}`)
						}
						reply('Transmiss茫o enviada com sucesso')
					}
					break
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse carinha aqui @${mentioned[0].split('@')[0]} agora 茅 admin ent茫o respeitem ok?! 馃槀`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse carinha aqui @${mentioned[0].split('@')[0]} Acabou de perder o adm pressionem F ai rapaziada 馃槀!`, mentioned, true)
				...
				case 'add':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (args.length < 1) return reply('quem voc锚 deseja adicionar, um g锚nio??')
					if (args[0].startsWith('08')) return reply('Use o c贸digo do pa铆s amigo')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('Falha ao adicionar a pessoa, talvez seja porque 茅 privado')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
				...
				case 'listadmins':
					...
					break
                                ...
                                case 'leave':
                                        ...
                                        break
				case 'toimg':
					if (!isQuotedSticker) return reply('鉂� responder sticker hum 鉂�')
					reply(mess.wait)
				...
				case 'simih':
					...
					if (Number(args[0]) === 1) {
						if (isSimi) return reply('O modo Simi est谩 ativado')
						samih.push(from)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Modo simi ativado com sucesso neste grupo hehe锔�')
					} else if (Number(args[0]) === 0) {
						samih.splice(from, 1)
						fs.writeFileSync('./src/simi.json', JSON.stringify(samih))
						reply('Modo simi desativado com sucesso neste grupo 鉁旓笍')
					} else {
						reply('1 para ativar, 0 para desativar, lerd茫o voc锚 em 馃う')
					}
					break
				case 'welcome':
					if (!isGroup) return reply(mess.only.group)
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('J谩 ativo um')
						welkom.push(from)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Ativou com sucesso o recurso de boas-vindas neste grupo 鉁旓笍')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('Desativou com sucesso o recurso de boas-vindas neste grupo 鉁旓笍')
					} else {
				...
				case 'clone':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('A tag alvo que voc锚 deseja clonar')
